...
public class Operation {
    ...
    public Operation(String name) {
			this(name, new TrueGuard(), new LinkedList<Action>(), 1);
		}
    public Operation(String name, Predicate guard) {
			this(name, guard, new LinkedList<Action>(), 1);
		}
    public Operation(String name, Action action) {
			this(name, new TrueGuard(), Arrays.asList(action), 1);
		}
    public Operation(String name, List < Action > actions) {
			this(name, new TrueGuard(), actions, 1);
		}
    public Operation(String name, Predicate guard, Action action) {
			this(name, guard, Arrays.asList(action), 1);
		}
    public Operation(String name, Predicate guard, List < Action > actions) {
			this(name, guard, actions, 1);
		}
